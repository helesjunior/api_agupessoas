    ...
    public function retornaApuracaoAntiguidade(Request $request)
    {
        ...
        try {
            $sql = DB::select("SELECT SERVIDOR.DS_CARGO_RH AS CARGO,
                                        ...
                                        (CASE WHEN SERVIDOR.NR_CLASSIFICACAO_CONCURSO = 0 THEN NULL ELSE SERVIDOR.NR_CLASSIFICACAO_CONCURSO END) AS CLASSIFICACAO_CONCURSO_PUBLICO,
                                        (CASE WHEN SERVIDOR.NR_ANO_CONCURSO = 0 THEN NULL ELSE SERVIDOR.NR_ANO_CONCURSO END) AS ANO_CONCURSO_PUBLICO,
                                        TO_CHAR(SERVIDOR.DT_NASCIMENTO, 'DD/MM/YYYY') AS DATA_NASCIMENTO,
                                        ROUND((SERVIDOR_DTC.TMP_CARREIRA - (CASE WHEN SERVIDOR.DIAS_AFASTADO IS NOT NULL THEN SERVIDOR.DIAS_AFASTADO ELSE 0 END)) / 365, 4) AS TEMPO_DE_EFETIVO_EXERCICIO,
                                        ...
                                        (SERVIDOR_DTC.TMP_CARREIRA - (CASE WHEN SERVIDOR.DIAS_AFASTADO IS NOT NULL THEN SERVIDOR.DIAS_AFASTADO ELSE 0 END)) AS APURACAO_DIAS_DE_EFET_EXERC,
                                        (CASE WHEN SERVIDOR.DIAS_AFASTADO IS NOT NULL THEN SERVIDOR.DIAS_AFASTADO ELSE 0 END) AS APURACAO_DIAS_AFASTADOS,
                                    ...
                                    FROM 
                                        (SELECT S.NM_SERVIDOR,
                                            ...
                                            AS DS_TIPO_PROVIMENTO,
                                                (SELECT 
                                                    SUM(
                                                        (CASE
                                                            WHEN A.DT_FIM_AFASTAMENTO > TO_DATE('05/04/2021', 'DD/MM/YYYY') THEN TO_DATE('05/04/2021', 'DD/MM/YYYY')
                                                ...
                                                WHERE TA.CD_TIPO_AFASTAMENTO IN ('1005504', '3161', '5000', '3101', '3104', '3118', '3133', '3136', '3137', '3142')
                                                    AND A.DT_INICIO_AFASTAMENTO<TO_DATE('05/04/2021', 'DD/MM/YYYY')
                                        ...
                                        WHERE
                                            ...
                                            AND C.CD_CARGO_RH IN ('408001', '408002', 'R408001', 'R408002')
                                        ) SERVIDOR,
                                        (SELECT S1.ID_SERVIDOR,
                                            MAX(CE1.DT_INGRESSO_SERVIDOR ) AS DT_INGRESSO_SERVIDOR,
                                            (TO_DATE('05/04/2021', 'DD/MM/YYYY') + 1 - MAX(CE1.DT_INGRESSO_SERVIDOR)) AS TMP_CARREIRA
