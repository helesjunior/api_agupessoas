...
', [$tpDocumento, $dtInicio]);
            ...
            $sql = DB::select("SELECT DISTINCT
                ...
                CASE SERVIDOR.CD_SEXO
                    WHEN 'M' THEN 'MASCULINO'
                         ELSE 'FEMININO'
                END as SEXO,
                LOT.CD_LOTACAO  AS \"CODIGO_UNIDADE_EXERCICIO\",
                UF.SG_UF        AS \"UF\",
                MU.NM_MUNICIPIO || ' - ' || UF.SG_UF AS \"CIDADE DA UNIDADE\",
                NI.DS_NIVEL                          AS \"NIVEL\",
                RJ.DS_REGIME_JURIDICO                AS \"REGIME JURIDICO\",
                TRIM(TIPO_SERVIDOR.DS_TIPO_SERVIDOR) AS \"SITUACAO FUNCIONAL\",
                LT.SG_ORGAO                          AS \"ORGAO DE ORIGEM\",
                DADO_FUNCIONAL.CD_MATRICULA_SIAPE,
                CASE
                WHEN SERVIDOR.IN_STATUS_SERVIDOR = 1 THEN
                'Ativo'
                else
                'Inativo'
                end Status
                from AGU_RH.SERVIDOR
                INNER JOIN AGU_RH.TIPO_SERVIDOR ON TIPO_SERVIDOR.id_tipo_servidor = SERVIDOR.id_tipo_servidor
                LEFT JOIN AGU_RH.DADO_FUNCIONAL ON DADO_FUNCIONAL.id_servidor = SERVIDOR.id_servidor
                LEFT JOIN DOCUMENTACAO D ON D.ID_SERVIDOR = SERVIDOR.ID_SERVIDOR AND D.ID_TIPO_DOCUMENTACAO = 1
                LEFT JOIN (
                SELECT U.ID_SERVIDOR, U.ULTIMA, N.ID_LOTACAO_EXERCICIO
                FROM (
                SELECT MAX(ID_MOVIMENTACAO) ULTIMA, ID_SERVIDOR
                FROM MOVIMENTACAO
                GROUP BY ID_SERVIDOR
                ...
                LEFT JOIN MOVIMENTACAO N ON N.ID_MOVIMENTACAO = U.ULTIMA
                LEFT JOIN ORGAO O ON O.ID_ORGAO = N.ID_ORGAO_MOVIMENTACAO
                ) MOV ON MOV.ID_SERVIDOR = SERVIDOR.ID_SERVIDOR
                LEFT JOIN AGU_RH.LOTACAO LOT ON LOT.ID_LOTACAO = MOV.ID_LOTACAO_EXERCICIO
                LEFT JOIN AGU_RH.ENDERECO EN ON EN.ID_ENDERECO = LOT.ID_ENDERECO
                LEFT JOIN AGU_RH.MUNICIPIO MU ON MU.ID_MUNICIPIO = EN.ID_MUNICIPIO
                LEFT JOIN AGU_RH.UF UF ON MU.ID_UF = UF.ID_UF
                LEFT JOIN AGU_RH.CARGO_EFETIVO CE ON CE.ID_SERVIDOR = SERVIDOR.ID_SERVIDOR
                LEFT JOIN AGU_RH.CARGO CA ON CA.ID_CARGO = CE.ID_CARGO
                LEFT JOIN NIVEL NI ON CA.ID_NIVEL = NI.ID_NIVEL
                LEFT JOIN AGU_RH.CESSAO CES ON CES.ID_SERVIDOR = SERVIDOR.ID_SERVIDOR
                LEFT JOIN AGU_RH.REGIME_JURIDICO RJ ON RJ.ID_REGIME_JURIDICO = DADO_FUNCIONAL.ID_REGIME_JURIDICO
                LEFT JOIN ( SELECT U.ID_SERVIDOR,
                O.DS_ORGAO || ' - ' || O.SG_ORGAO AS SG_ORGAO
                 FROM (
                SELECT MAX(ID_MOVIMENTACAO) ULTIMA, ID_SERVIDOR
                ...
                LEFT JOIN MOVIMENTACAO N ON N.ID_MOVIMENTACAO = U.ULTIMA
                LEFT JOIN ORGAO O ON O.ID_ORGAO = N.ID_ORGAO_MOVIMENTACAO
            ) LT ON LT.ID_SERVIDOR = SERVIDOR.ID_SERVIDOR
                 WHERE SERVIDOR.DT_OPERACAO_EXCLUSAO IS NULL
                 AND DADO_FUNCIONAL.DT_OPERACAO_EXCLUSAO IS NULL
     ...
     *
     * @return array
     * @author Juliano Piress <juliano.santos@agu.gov.br>
     */
    public function retornaDadosAtualizaPpiPgd($cpf)
    {
        ini_set("memory_limit", "512M");
        ini_set("set_time_limit", "600");

        $sql = "SELECT DISTINCT
    DF.CD_MATRICULA_SIAPE AS MATRICULA_SIAPE,
    --S.ID_SERVIDOR,
    DOC.NR_DOCUMENTACAO AS CPF,
    LTRIM(S.NM_SERVIDOR) AS NOME_SERVIDOR,
	S.DT_NASCIMENTO AS DATA_DE_NASCIMENTO,
    S.NM_EMAIL_INSTITUCIONAL AS EMAIL_INSTITUCIONAL,
	CAR.CD_CARGO_RH AS CODIGO_CARGO,
    CAR.DS_CARGO_RH  AS CARGO_FUNCAO,
   MOV.ID_LOTACAO_ORIGEM,
   LOT1.ID_LOTACAO AS ID_UNIDADE_LOTACAO,
    LOT1.CD_LOTACAO AS CODIGO_UNIDADE_LOTACAO,
    LOT1.SG_LOTACAO AS SIGLA_UNIDADE_LOTACAO,
    LOT1.DS_LOTACAO AS UNIDADE_LOTACAO,
   MOV.ID_LOTACAO_EXERCICIO,
	LOT2.ID_LOTACAO AS ID_UNIDADE_EXERCICIO,
    LOT2.CD_LOTACAO AS CODIGO_UNIDADE_EXERCICIO,
    LOT2.SG_LOTACAO AS SIGLA_UNIDADE_EXERCICIO,
    LOT2.DS_LOTACAO AS UNIDADE_DE_EXERCICIO,
    M.NM_MUNICIPIO || ' - ' || UF.SG_UF AS CIDADE_UNIDADE_EXERCICIO,
    O.SG_ORGAO                          AS SIGLA_ORGAO_ORIGEM,
    O.DS_ORGAO                          AS ORGAO_ORIGEM,
	LP1.ID_LOTACAO AS ID_UNIDADE_01,
	LP1.CD_LOTACAO AS CODIGO_UNIDADE_01,
	LP1.DS_LOTACAO AS DS_EXERCICIO_PAI01,
    LP1.SG_LOTACAO AS SG_EXERCICIO_PAI01,
	LP2.ID_LOTACAO AS ID_UNIDADE_02,
	LP2.CD_LOTACAO AS CODIGO_UNIDADE_02,
    LP2.DS_LOTACAO AS DS_EXERCICIO_PAI02,
    LP2.SG_LOTACAO AS SG_EXERCICIO_PAI02,
	LP3.ID_LOTACAO AS ID_UNIDADE_03,
	LP3.CD_LOTACAO AS CDIGO_UNIDADE_03,
    LP3.DS_LOTACAO AS DS_EXERCICIO_PAI03,
    LP3.SG_LOTACAO AS SG_EXERCICIO_PAI03,
	LP4.ID_LOTACAO AS ID_DA_UNIDADE_04,
	LP4.CD_LOTACAO AS CODIGO_UNIDADE_04,
    LP4.DS_LOTACAO AS DS_EXERCICIO_PAI04,
    LP4.SG_LOTACAO AS SG_EXERCICIO_PAI04,
	LP5.ID_LOTACAO AS ID_UNIDADE_05,
	LP5.CD_LOTACAO AS CODIGO_UNIDADE_05,
    LP5.DS_LOTACAO AS DS_EXERCICIO_PAI05,
    LP5.SG_LOTACAO AS SG_EXERCICIO_PAI05

FROM
    AGU_RH.MOVIMENTACAO MOV
        LEFT JOIN AGU_RH.SERVIDOR S ON S.ID_SERVIDOR = MOV.ID_SERVIDOR AND S.IN_STATUS_SERVIDOR = 1 AND S.DT_OPERACAO_EXCLUSAO IS NULL
        LEFT JOIN AGU_RH.DADO_FUNCIONAL DF ON DF.ID_SERVIDOR = S.ID_SERVIDOR
        LEFT JOIN AGU_RH.DOCUMENTACAO DOC ON S.ID_SERVIDOR = DOC.ID_SERVIDOR AND DOC.ID_TIPO_DOCUMENTACAO = 1
        LEFT JOIN AGU_RH.LOTACAO LOT1 ON LOT1.ID_LOTACAO = MOV.ID_LOTACAO_ORIGEM
        LEFT JOIN AGU_RH.LOTACAO LOT2 ON LOT2.ID_LOTACAO = MOV.ID_LOTACAO_EXERCICIO
		LEFT JOIN AGU_RH.LOTACAO LP1 ON LOT2.ID_LOTACAO_PAI = LP1.ID_LOTACAO
    LEFT JOIN AGU_RH.LOTACAO LP2 ON LP1.ID_LOTACAO_PAI = LP2.ID_LOTACAO
    LEFT JOIN AGU_RH.LOTACAO LP3 ON LP2.ID_LOTACAO_PAI = LP3.ID_LOTACAO
    LEFT JOIN AGU_RH.LOTACAO LP4 ON LP3.ID_LOTACAO_PAI = LP4.ID_LOTACAO
    LEFT JOIN AGU_RH.LOTACAO LP5 ON LP4.ID_LOTACAO_PAI = LP5.ID_LOTACAO
        LEFT JOIN AGU_RH.ENDERECO E ON E.ID_ENDERECO = LOT2.ID_ENDERECO
        LEFT JOIN AGU_RH.UF UF ON UF.ID_UF =  E.ID_UF_ENDERECO
        LEFT JOIN AGU_RH.MUNICIPIO M ON M.ID_MUNICIPIO = E.ID_MUNICIPIO
        JOIN AGU_RH.CARGO_EFETIVO C ON ID_CARGO_EFETIVO = (SELECT MAX (ID_CARGO_EFETIVO) FROM AGU_RH.CARGO_EFETIVO WHERE ID_SERVIDOR = S.ID_SERVIDOR) AND C.DT_OPERACAO_EXCLUSAO IS NULL
        LEFT JOIN AGU_RH.CARGO CAR ON CAR.ID_CARGO = C.ID_CARGO
        LEFT JOIN AGU_RH.ORGAO O ON O.ID_ORGAO = MOV.ID_ORGAO_MOVIMENTACAO

WHERE
        DOC.NR_DOCUMENTACAO = $cpf
        AND
        DT_FINAL_MOVIMENTACAO IS NULL
        --AND CAR.CD_CARGO_RH NOT IN ('11002','410004','414001','410001','410002','600001','600002','408001','408002','77777000','414017','411001') FILTRO PAR EXCLUIR OS MEMBROS
        ORDER BY 3 ASC";
