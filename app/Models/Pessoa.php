...
FROM (
    ...
    public function retornaAfastamentoServidor($tpDocumento, $dtInicio)
    {
     ...
     , TO_CHAR(DT_MOV, 'DD/MM/YYYY') as DT_MOV
     , TO_CHAR(DT_INICIO_MOVIMENTACAO, 'DD/MM/YYYY') as DT_INICIO_MOVIMENTACAO
     , \"SITUACAO FUNCIONAL\"
     , \"ORGAO DE ORIGEM\"
        ...
        try {
            ...
            $sql = DB::select("WITH cargo_eftv as (
                                        ...
                                        SELECT DISTINCT CE1.ID_CARGO_EFETIVO,
                                               ...
                                               TO_CHAR(SYSDATE, 'DD/MM/YYYY') as DATA_FINAL,
                                               MOV.NOME_SERVIDOR,
            ...
            $sql = DB::select("SELECT DISTINCT
                                     ...
                                     FROM VW_REL_CARGOEFETIVO CAR
                                         ...
                                         MOV ON MOV.ID_SERVIDOR = SER.ID_SERVIDOR
                                             ...
                                             LT.ID_SERVIDOR = SER.ID_SERVIDOR
                                     WHERE DAD.id_rh = 1 and CAR.ANO_CONCURSO is not null");
            DB::commit();
        ...
        try {
            ...
            $sql = DB::select("
                                        SELECT
                                                ...
                                                CASE
                                                    WHEN LOTACAO.IN_TIPO_NORMA_UDP = 'I'
                                                        THEN 'Sim'
                                                    ELSE 'Não'
                                                END  AS UDP,
                                                CASE
                                                    WHEN LOTACAO.IN_TIPO_NORMA_ODS = 'I'
                                                        THEN 'Sim'
                                                    ELSE 'Não'
                                                END  AS ODS
